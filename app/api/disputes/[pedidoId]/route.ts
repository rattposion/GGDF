...
export async function POST(request: NextRequest, { params }: { params: { pedidoId: string } }) {
  try {
    ...
    const { reason } = body
    if (!reason) {
      return NextResponse.json(createError('Motivo obrigatório', 400), { status: 400 })
    }
    ...
    const existing = await prisma.dispute.findUnique({ where: { orderId: pedidoId } })
    if (existing) {
      return NextResponse.json(createError('Disputa já aberta para este pedido', 400), { status: 400 })
    }
    const dispute = await prisma.dispute.create({ data: { orderId: pedidoId, userId: session.user.id, reason, status: 'OPEN' } })
