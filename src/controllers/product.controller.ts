...
export const deleteProduct = async (req: Request, res: Response) => {
  try {
    ...
    const sellerId = (req as any).userId;
    // Buscar o produto para validar o dono
    ...
    const order = await prisma.order.findFirst({ where: { productId: id } });
    if (order) {
      // Apenas inativar
      await prisma.product.update({ where: { id }, data: { isActive: false } });
      return res.json({ success: true, message: 'Produto arquivado/inativado com sucesso.' });
    } else {
      // Pode excluir
    ...
    console.error('Erro ao deletar/inativar produto:', err);
    res.status(500).json({ error: 'Erro ao deletar/inativar produto.', details: err });
  }
};
